...
#    没出错就传递id继续执行
try:
	response = urllib2.urlopen(url='http://pso2.jp/players/news/maintenance/')
...
if (citem) and (date <= mtncanceldate):
	try:
		response = urllib2.urlopen(url='http://pso2.jp/players/news/'+citem[0][0]+'/')
		content = response.read()
...
if (item) and (date <= mtninfodate):
	try:
		response = urllib2.urlopen(url='http://pso2.jp/players/news/'+item[0][0]+'/')
		content = response.read()
...
#    没出错就传递id继续执行
try:
	response = urllib2.urlopen(url='http://pso2.jp/players/news/information/')
	content = response.read()
...
if (item) and (date <= uinfodate):
	try:
		response = urllib2.urlopen(url='http://pso2.jp/players/news/?id='+item[0][0])
		content = response.read()
...
with open( sys.path[0] + '/mtnupdate.json', 'w') as f:
